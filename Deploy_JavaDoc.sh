...
if [ "$TRAVIS_REPO_SLUG" == "Valkryst/VTemperature" ] &&
   ...
   [ "$TRAVIS_BRANCH" == "master" ]; then
    ...
    cd $HOME
    git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/Valkryst/VTemperature gh-pages > /dev/null
    
    cd gh-pages
    git rm -r *
    git config user.name "Valkryst"
    git config user.email "valkryst@valkryst.com"
    cp -r /home/travis/build/Valkryst/VTemperature/target/site/apidocs/* .
    git add .
